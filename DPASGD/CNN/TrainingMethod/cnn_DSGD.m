function [opttheta, cost_e, time] = cnn_DSGD(funObj, cnn_distributed, training, Graph, n_agents, options)
...
for e = 1:epochs
    ...
    for s = 1:minibatch:(m-minibatch+1)
            ...
            theta_matrix = theta_matrix - gamma*grad_matrix;
    ...
    theta_matrix = Graph*theta_matrix + inv(Graph)*gamma*grad_matrix;
    opttheta(e, :) =  sum(theta_matrix,1)/n_agents;
end
% opttheta =  sum(theta_matrix,1)/n_agents;
time = toc;
fprintf('DSGD done, and the running time is %d s.\n', time);
end
